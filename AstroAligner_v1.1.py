...
class AstroAligner:
    def __init__(self):
        print("AstroAligner v1.7 - Fixed comet mode click handling (indentation bug)")
        self.root = tk.Tk()
    ...
    def on_set_ref(self):
        ...
        if mode == "stars":
            ...
            ann = self.comet_ann[self.current_idx]
            if ann is None:
                messagebox.showwarning("Warning", "Current image must have comet annotation to set as reference.")
                self.set_ref_var.set(False)
                return
            cx, cy = ann["center"]
            r = ann.get("radius", 0.0)
            tx, ty = ann["tail_dir"]
            vec_len = math.hypot(tx - cx, ty - cy)
            if r <= 0 or vec_len <= 10.0:
                messagebox.showwarning("Warning", "Comet must have center (radius >0) and tail (>10px) to set as reference.")
                self.set_ref_var.set(False)
                return
    ...
    def load_comet_from_file(self, directory):
        if not directory:
            return 0
        comet_file = os.path.join(directory, "comet_annotations.txt")
        if not os.path.exists(comet_file):
            print("Comet file not found: comet_annotations.txt")
            return 0
        loaded = 0
            ...
            for line in f:
                ...
                if len(parts) >= 6:  # basename cx cy r tx ty
                    ...
                    try:
                        ...
                        for idx, file_path in enumerate(self.raw_files):
                            basename_match = os.path.splitext(os.path.basename(file_path))[0]
                            if basename_match == basename:
                                self.comet_ann[idx] = {"center": (cx, cy), "radius": r, "tail_dir": (tx, ty)}
                                loaded += 1
                                print(f"Loaded comet for {basename} at index {idx}")
                                break
                    except (ValueError, IndexError):
                        print(f"Invalid line in comet file: {line}")
                        pass
        print(f"Loaded {loaded} comet annotations")
        return loaded
    ...
    def finalize_loading(self, raw_files):
        ...
        loaded_comet = self.load_comet_from_file(self.input_dir)
        ...
        self.update_indicators()
        # Force reset checkbox after load to avoid auto-activation
        self.set_ref_var.set(False)
    ...
    def redraw_points(self):
        ...
        if mode == "stars":
            ...
            if ann:
                ...
                # Cross at center (fixed: content_cy for horizontal line)
    ...
    def align_and_save(self):
        ...
        if mode == "comet":
            ...
            if ref_ann is None:
                messagebox.showerror("Error", "Reference must have comet annotation (center + tail).")
                return
